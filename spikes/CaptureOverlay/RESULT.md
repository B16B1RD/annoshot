...
無人実行では終了コードに加えて `measurements.md` 内の上記判定行を確認すること（`計測不能` とカーソルの非対応行には `**` が付かない。
単一 DPI 環境の `**未計測（DPI 混在なし）**` は想定内）。WGC 経路（`--gdi` なし）では加えて「D3D11 ドライバ」行が `HARDWARE` であることを
確認する（`--gdi` 指定時はこの行は出力されない）。
